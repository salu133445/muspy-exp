...
(LOG_DIR / "test" / "combined-vs-combined").mkdir(exist_ok=True)
log_file = open(
    str(LOG_DIR / "test" / "combined-vs-combined" / "cross-dataset-test.log"),
    "w",
)
...
for src_dataset in ["combined", "combined_stratified"]:
    for tgt_dataset in ["combined", "combined_stratified"]:

        print("-" * 80)
        print("Using model trained on dataset {}".format(src_dataset))
        model_filename = str(
            MODEL_DIR / src_dataset / "{}.pt".format(args.model)
        )
