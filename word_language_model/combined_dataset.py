...
parser.add_argument(
    "--stratified",
    action="store_true",
    help="whether to apply stratified sampling",
)
...
for dataset_key in DATASET_KEYS:
    music_dataset = muspy.MusicDataset(DATASET_DIR / dataset_key)
    split_filename = DATASET_DIR / dataset_key / "splits.txt"
    # if not split_filename.is_file():
    ...
    train_datasets.append(pytorch_datasets["train"])
    test_datasets.append(pytorch_datasets["test"])
    val_datasets.append(pytorch_datasets["validation"])


if args.stratified:

    model_name = "combined_stratified"

    class StratifiedCombinedDataset(Dataset):
        def __init__(self, datasets):
            self.datasets = datasets
            self.length = sum(len(x) for x in datasets)
        ...
        def __getitem__(self, index):
            ...
            return dataset[rand_index]

    train_data = DataLoader(
        StratifiedCombinedDataset(train_datasets),
    ...
    val_data = DataLoader(
        StratifiedCombinedDataset(val_datasets),
...
model_filename = str(MODEL_DIR / model_name / "{}.pt".format(args.model))

(LOG_DIR / model_name).mkdir(exist_ok=True)
(LOG_DIR / model_name / args.model).mkdir(exist_ok=True)
log_file = open(str(LOG_DIR / model_name / args.model / "train.log"), "w")
val_log_file = open(
    str(LOG_DIR / model_name / args.model / "validation.log"), "w"
)
