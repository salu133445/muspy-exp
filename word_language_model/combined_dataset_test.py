...
model_name = "combined_stratified" if args.stratified else "combined"

(LOG_DIR / "test").mkdir(exist_ok=True)
(LOG_DIR / "test" / model_name).mkdir(exist_ok=True)
log_file = open(
    str(LOG_DIR / "test" / model_name / "cross-dataset-test.log"), "w"
)
...
src_dataset = model_name

for tgt_dataset in DATASET_KEYS:

    print("-" * 80)
    print("Using model trained on COMBINED dataset")
    model_filename = str(MODEL_DIR / src_dataset / "{}.pt".format(args.model))
